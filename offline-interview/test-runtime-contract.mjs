...
assert.match(app, /interview-runtime-v41\.10/);
...
assert.match(sw, /offline-interview-v41\.10/);
assert.match(index, /styles\.css\?v=41\.10/);
assert.match(index, /app\.js\?v=41\.10/);
...
// V41.10: automatic transcription is system-first/system-only; saved audio remains usable when text is absent.
...
// V41.10 field stabilization: explicit capture ownership, gapless semantic boundaries, replay pause, idempotent retranscription.
...
// V41.10: semantic boundary text is derived from immutable saved-audio intervals, not cross-boundary live hypotheses.
...
// V41.10: live system text is committed immediately; saved-audio retranscription is recovery, never a critical-path prerequisite.
...
// V41.10 hybrid audio: master remains authoritative; context is a derived window only.
...
console.log(JSON.stringify({
  status: 'PASS',
  contract: 'offline-interview.runtime-contract.v41.10',
  appBytes: bytes(app),
  cssBytes: bytes(css),
  coreBytes
}));

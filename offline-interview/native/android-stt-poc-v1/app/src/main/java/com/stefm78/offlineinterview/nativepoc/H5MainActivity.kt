...
/** H5 = H4 nonblocking audio stability + bounded asynchronous STT finalization. */
class H5MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            ...
            status.text = if (SpeechRecognizer.isOnDeviceRecognitionAvailable(this)) "Prêt H5 — STT on-device disponible." else "HOLD: reconnaissance on-device indisponible."
        }
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            REQ_OPEN_SPEC -> data?.data?.let { uri ->
                try { loadSpecFromUri(uri); status.text = "Questionnaire chargé: ${interviewSpec.title}." }
                catch (e: Exception) { status.text = "Questionnaire refusé: ${e.message}" }
            }
            REQ_SAVE_RESULT -> data?.data?.let { uri ->
                lastExportJson?.let { payload -> executor.execute {
                    val msg = try { contentResolver.openOutputStream(uri, "wt")?.bufferedWriter()?.use { it.write(payload) }; "Résultat JSON enregistré." }
    ...
    private fun startSession() {
        ...
        transcriptEvents.clear(); canonicalText.clear(); latestPartialByTurn.clear(); durableDraftByTurn.clear(); draftSourceByTurn.clear()
        sttSessions.clear(); sttSessionHistory.clear(); lastTelemetryPartialText.clear(); lastTelemetryPartialAt.clear()
        synchronized(pendingLock) { pendingPcm.clear(); pendingTurn = null }
        activeSttSession = null
        droppedTranscriptEventCount = 0; droppedSttPcmChunks = 0; finalizationFallbackCount = 0; sttDegraded = false; bytesWritten = 0L; lastExportJson = null
    ...
    private fun sttFeederLoop(s: TurnSttSession) {
        try {
            ...
            if (!s.closed && !s.finalizing) runOnUiThread { status.text = "STT pipe ${questions[s.turn].id} fermé — WAV continue." }
        } finally {
    ...
    private fun captureLoop() {
        ...
        while (recording.get()) {
            ...
            if (n <= 0) continue
            try { wavRaf?.write(buffer, 0, n); bytesWritten += n }
            catch (_: IOException) { runOnUiThread { status.text = "FAIL_MASTER: WAV interrompu." }; recording.set(false); break }
    ...
    private fun snapshotPartialAtClose(turn: Int) {
        if (durableDraftByTurn[turn].isNullOrBlank()) {
            latestPartialByTurn[turn].orEmpty().trim().takeIf { it.isNotEmpty() }?.let {
                ...
                draftSourceByTurn[turn] = "partial_snapshot_at_finalize_timeout"
            }
        }
    }
    ...
    private fun scheduleTransientRearm(s: TurnSttSession, error: Int): Boolean {
        ...
        mainHandler.postDelayed({
            if (recording.get() && currentTurn == s.turn && activeSttSession == null) createAndActivateSession(s.turn, now, s.attempt + 1)
    ...
    private fun lockCurrentDraft() {
        ...
        if (draft.isEmpty()) { status.text = "Aucun brouillon STT à valider."; return }
    ...
    private fun stopSession() {
        ...
        if (s != null) requestFinalization(s, stopAt, "interview_stop") { finalizeInterviewAsync() }
        else finalizeInterviewAsync()
    }
    ...
    private fun finalizeInterviewAsync() {
        executor.execute {
            ...
            runOnUiThread {
                ...
                status.text = if (sttDegraded) "STOPPED — dégradation STT observée." else "STOPPED H5 — WAV finalisé."
            }
        }
    }
    ...
    private fun appendEvent(e: TranscriptEvent, partial: Boolean = false) {
        ...
        if (partial) {
            ...
            if (pt == e.text || e.callbackAtMs - pa < PARTIAL_TELEMETRY_MIN_INTERVAL_MS) { droppedTranscriptEventCount++; return }
    ...
    private fun recordPartial(s: TurnSttSession, text: String) {
        ...
        appendEvent(TranscriptEvent(elapsedMs(), null, s.turn, "partial", "stt_session_identity", clean), true)
    ...
    private inner class TurnRecognitionListener(private val s: TurnSttSession) : RecognitionListener {
        ...
        override fun onError(error: Int) {
            if (s.finalizing) {
                s.providerError = error
                s.providerErrorClass = "finalization_provider_error"
                appendEvent(TranscriptEvent(elapsedMs(), null, s.turn, "finalization_error", "stt_session_identity", "error=$error"))
            ...
            sttDegraded = true
            appendEvent(TranscriptEvent(elapsedMs(), null, s.turn, "error", "stt_session_identity", "error=$error"))
            if (activeSttSession === s) activeSttSession = null
        }
        override fun onResults(results: Bundle?) {
            recordDurable(s, results, results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty(), "final")
            if (s.finalizing) completeFinalization(s, "provider_final")
        }
        override fun onPartialResults(partialResults: Bundle?) {
            recordPartial(s, partialResults?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty())
        }
        override fun onEvent(eventType: Int, params: Bundle?) {}
        override fun onSegmentResults(segmentResults: Bundle) {
            recordDurable(s, segmentResults, segmentResults.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty(), "segment")
        }
        override fun onEndOfSegmentedSession() {
            if (s.finalizing) completeFinalization(s, if (s.segmentCount > 0) "provider_segment" else "segmented_end_partial_fallback")
        }
    }
    ...
    private fun buildProductResult(): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("provenance", JSONObject().apply {
                ...
                put("transcriptionFallback", "partial_snapshot_after_bounded_finalization")
            ...
            put("nativeCapture", JSONObject().apply {
                put("schema", "offline-interview.android-native-runtime.v5.0")
                put("appVersion", BuildConfig.VERSION_NAME)
                put("audioAuthority", "single_AudioRecord_PCM_to_WAV")
                put("sttProvider", "Android SpeechRecognizer on-device via H4 bounded nonblocking feeder + H5 bounded asynchronous finalization")
                put("routingAuthority", "stt_session_identity")
                put("wavPath", wavFile?.absolutePath ?: "")
                put("pcmBytes", bytesWritten); put("audioDurationMs", durationMs); put("sttDegraded", sttDegraded)
                put("transientRearmMax", MAX_TRANSIENT_REARMS); put("finalizationGraceMs", FINALIZATION_GRACE_MS); put("finalizationFallbackCount", finalizationFallbackCount)
                ...
                put("sttSessions", JSONArray().apply { sttSessionHistory.forEach { s -> put(JSONObject().apply {
                    ...
                    put("endAtMs", s.endAtMs ?: JSONObject.NULL); put("closeReason", s.closeReason ?: JSONObject.NULL)
                    put("partialCount", s.partialCount); put("finalCount", s.finalCount); put("segmentCount", s.segmentCount); put("timedPartCount", s.timedPartCount)
                    put("providerError", s.providerError ?: JSONObject.NULL); put("providerErrorClass", s.providerErrorClass ?: JSONObject.NULL); put("droppedPcmChunks", s.droppedPcmChunks)
                    put("finalizationRequestedAtMs", s.finalizationRequestedAtMs ?: JSONObject.NULL); put("finalizationCompletedAtMs", s.finalizationCompletedAtMs ?: JSONObject.NULL)
    ...
    override fun onDestroy() {
        ...
        sttSessionHistory.forEach { s ->
            s.feederRunning.set(false); s.feederThread?.interrupt()
            try { s.sink.close() } catch (_: Exception) {}
            try { s.readFd.close() } catch (_: Exception) {}
            try { s.recognizer.destroy() } catch (_: Exception) {}
        }

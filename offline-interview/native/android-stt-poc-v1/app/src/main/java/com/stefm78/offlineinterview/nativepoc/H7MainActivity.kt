...
/**
 * H7 restores the segmented-session flag that was present in the original field-proven V1
 * (`EXTRA_SEGMENTED_SESSION = EXTRA_AUDIO_SOURCE`) while preserving H4/H6 invariants:
 * one authoritative AudioRecord/WAV master, disposable nonblocking STT feeder, per-turn
 * recognizer identity, EOF-driven finalization, provider cooldown and human-lock precedence.
 *
 * It also fixes H6 observability loss for the first partial event and expands the bounded STT
 * queue so provider startup backpressure cannot immediately discard several seconds of Q01 PCM.
 */
class H7MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            ...
            status.text = if (SpeechRecognizer.isOnDeviceRecognitionAvailable(this))
                "Prêt H7 — STT on-device segmented disponible."
    ...
    private fun buildUi() {
        ...
        status = TextView(this).apply { text = "Initialisation H7…" }
    ...
    private fun baseRecognizerIntent(audioSource: ParcelFileDescriptor?): Intent =
        Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            if (audioSource != null) {
                ...
                // Restored from the original V1 that produced segment callbacks in the field.
    ...
    private fun startSession() {
        ...
        status.text = "RUNNING H7 — WAV maître; STT segmented par question."
    }
    ...
    private fun createAndActivateSession(turn: Int, startAtMs: Long, attempt: Int): TurnSttSession? {
        ...
        return try {
            ...
            s.feederThread = Thread(
                { sttFeederLoop(s) },
                "offline-interview-stt-${questions[turn].id}-a$attempt"
            ).also { it.start() }
    ...
    private fun offerSttChunk(s: TurnSttSession, chunk: ByteArray) {
        if (!s.pcmQueue.offer(chunk)) {
            s.droppedPcmChunks++
            droppedSttPcmChunks++
            return
        }
        val depth = s.pcmQueue.size
        if (depth > s.queueHighWaterMark) s.queueHighWaterMark = depth
    }
    ...
    private fun flushPendingPcm(s: TurnSttSession) {
        ...
        chunks.forEach { offerSttChunk(s, it) }
    }
    ...
    private fun nextTurn() {
        ...
        status.text = "H7 ${questions[newTurn].id} affichée — finalisation segmented ${questions[oldTurn].id}."
    ...
    private fun requestFinalization(
        ...
        // Let the feeder drain queued PCM, then close the external source. Segmented-session mode
        // tells the provider that source closure is the session boundary.
        if (!s.pcmQueue.offer(POISON)) {
            s.pcmQueue.poll()
            if (!s.pcmQueue.offer(POISON)) s.feederThread?.interrupt()
        }
        mainHandler.postDelayed({
            if (s.finalizing) completeFinalization(s, "segmented_eof_timeout_partial_fallback")
    ...
    private fun snapshotPartialAtClose(turn: Int) {
        if (durableDraftByTurn[turn].isNullOrBlank()) {
            latestPartialByTurn[turn].orEmpty().trim().takeIf { it.isNotEmpty() }?.let {
                ...
                draftSourceByTurn[turn] = "partial_snapshot_at_segmented_eof_timeout"
            }
        }
    }
    ...
    private fun finalizeInterviewAsync() {
        executor.execute {
            ...
            runOnUiThread {
                exportView.text = "Résultat prêt — $answered/${questions.size} réponses · fallback=$finalizationFallbackCount."
                loadButton.isEnabled = true
                startButton.isEnabled = true
                saveButton.isEnabled = true
                status.text = if (sttDegraded) "STOPPED — dégradation STT observée." else "STOPPED H7 — WAV finalisé."
            }
        }
    }
    ...
    private fun appendEvent(e: TranscriptEvent, partial: Boolean = false) {
        ...
        if (partial) {
            val previousText = lastTelemetryPartialText[e.turn]
            val previousAt = lastTelemetryPartialAt[e.turn]
            if (previousText == e.text || (previousAt != null && e.callbackAtMs - previousAt < PARTIAL_TELEMETRY_MIN_INTERVAL_MS)) {
                droppedTranscriptEventCount++
                return
            }
    ...
    private inner class TurnRecognitionListener(private val s: TurnSttSession) : RecognitionListener {
        ...
        override fun onError(error: Int) {
            if (s.finalizing) {
                s.providerError = error
                s.providerErrorClass = "segmented_eof_finalization_provider_error"
                appendEvent(
                    TranscriptEvent(
                        elapsedMs(), null, s.turn, "finalization_error", "stt_session_identity", "error=$error"
                    )
                )
    ...
    private fun buildProductResult(): JSONObject {
                    ...
                    if (flat >= 0) {
                        ...
                        if (text.isNotEmpty()) {
                            turns.put(JSONObject().apply {
                                ...
                                put("transcriptionSource", "android-on-device-per-turn-segmented")
        ...
        return JSONObject().apply {
            ...
            put("provenance", JSONObject().apply {
                ...
                put("transcriptionDefault", "android-on-device-per-turn-segmented")
                put("transcriptionFallback", "partial_snapshot_after_bounded_segmented_eof_finalization")
            ...
            put("nativeCapture", JSONObject().apply {
                put("schema", "offline-interview.android-native-runtime.v7.0")
                put("appVersion", BuildConfig.VERSION_NAME)
                put("audioAuthority", "single_AudioRecord_PCM_to_WAV")
                put("sttProvider", "Android SpeechRecognizer on-device via per-turn EXTRA_AUDIO_SOURCE + EXTRA_SEGMENTED_SESSION")
                ...
                put("segmentedSessionMode", "EXTRA_SEGMENTED_SESSION=EXTRA_AUDIO_SOURCE")
                put("finalizationMethod", "segmented_pipe_eof_no_stopListening")
                put("finalizationGraceMs", FINALIZATION_GRACE_MS)
                put("providerCooldownMs", PROVIDER_COOLDOWN_MS)
                put("sttQueueCapacityChunks", STT_QUEUE_CHUNKS)
                put("pendingPcmCapacityChunks", PENDING_PCM_CHUNKS)
                ...
                put("sttSessions", JSONArray().apply {
                    sttSessionHistory.forEach { s ->
                        put(JSONObject().apply {
                            ...
                            put("queueHighWaterMark", s.queueHighWaterMark)
    ...
    companion object {
        ...
        private const val FINALIZATION_GRACE_MS = 1500L
        ...
        private const val STT_QUEUE_CHUNKS = 128
        private const val PENDING_PCM_CHUNKS = 32
        private const val MAX_TRANSCRIPT_EVENTS = 600

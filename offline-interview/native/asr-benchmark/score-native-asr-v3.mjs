...
function equivalentEntityHit(raw, entity, corpus, policy) {
  ...
  const hyp = ` ${normalizeForWerV3(raw, corpus, policy)} `;
  const canonical = ` ${normalizeForWerV3(entity.canonical, corpus, policy)} `;
...
function semanticForPassage(passageId, hypothesisRaw, policy) {
  ...
  const details = checks.map(c => {
    ...
    const state = contradiction ? 'CONTRADICTED' : hit ? 'PRESENT_CORRECT' : 'MISSING';
    if (state === 'PRESENT_CORRECT') present++;
    else if (state === 'CONTRADICTED') contradicted++;
    else missing++;
    return { id: c.id, state, hit, contradiction };
  });
  return { present, missing, contradicted, total: checks.length, details };
}

export function scoreV3(corpus, policy, hypotheses) {
  ...
  const passages = (corpus.passages || []).map(p => {
    ...
    const referenceNormalized = normalizeForWerV3(p.reference, corpus, policy);
    const hypothesisNormalized = normalizeForWerV3(hypothesisRaw, corpus, policy);
  ...
  const criticalMeaningPresent = passages.reduce((n, p) => n + p.semantic.present, 0);
  const criticalMeaningMissing = passages.reduce((n, p) => n + p.semantic.missing, 0);
  const criticalMeaningContradicted = passages.reduce((n, p) => n + p.semantic.contradicted, 0);
  const criticalMeaningTotal = passages.reduce((n, p) => n + p.semantic.total, 0);
  const criticalMeaningAccuracy = criticalMeaningTotal ? criticalMeaningPresent / criticalMeaningTotal : 1;
  ...
  const completedPassages = passages.filter(p => p.userFinished && p.errorCode == null && p.hypothesisRaw.trim()).length;
  const lowCoveragePassages = passages.filter(p => p.wordCoverageRatio < 0.95).length;
  const allCoverageAtLeast95 = lowCoveragePassages === 0;
  const allUserConfirmed = userConfirmedPassages === passages.length;
  const noCriticalMeaningLoss = criticalMeaningMissing === 0 && criticalMeaningContradicted === 0;

  let verdict;
  if (allUserConfirmed && blockingErrors === 0 && allCoverageAtLeast95 && globalWer <= 0.10 && criticalEntityAccuracy >= 0.90 && criticalMeaningAccuracy === 1 && noCriticalMeaningLoss) {
    verdict = 'PASS_NATIVE_ASR';
  } else if (allUserConfirmed && blockingErrors === 0 && allCoverageAtLeast95 && globalWer <= 0.15 && criticalEntityAccuracy >= 0.80 && criticalMeaningAccuracy === 1 && noCriticalMeaningLoss) {
    verdict = 'PASS_WITH_LIMITATIONS';
  } else if (allUserConfirmed && blockingErrors === 0 && (lowCoveragePassages >= 2 || globalWer > 0.15 || !noCriticalMeaningLoss)) {
    verdict = 'PIVOT_NATIVE_ENGINE';
  } else {
    verdict = 'FAIL_NATIVE_ASR';
  }

  return {
    schema: 'offline-interview.native-asr-benchmark-score.v3',
    corpusId: corpus.id,
    ...
    aggregate: {
      ...
      criticalMeaningPresent,
      criticalMeaningMissing,
      criticalMeaningContradicted,
      criticalMeaningTotal,
      criticalMeaningAccuracy,
      lowCoveragePassages,
      allCoverageAtLeast95,
      blockingErrors,
      verdict
    }
  };
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (!corpusPath || !policyPath || !hypothesesPath) {
    console.error('Usage: node score-native-asr-v3.mjs <corpus.json> <policy.json> <hypotheses.json>');
    process.exit(2);
  }
  ...
  console.log(JSON.stringify(scoreV3(corpus, policy, hypotheses), null, 2));
}

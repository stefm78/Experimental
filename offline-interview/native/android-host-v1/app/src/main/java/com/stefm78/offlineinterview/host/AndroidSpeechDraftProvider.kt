...
/**
 * LIVE_DRAFT_ONLY provider extracted from the Android native V3 benchmark trajectory.
 * Product/session/audio authority remains in the Web application.
 */
class AndroidSpeechDraftProvider(
    ...
    companion object {
        const val PROVIDER_ID = "ANDROID_SYSTEM_DEFAULT_V3_DRAFT"
        const val PROVIDER_MODE = "android-system-default-v3-draft"
        ...
        private const val PRODUCT_STOP_GRACE_MS = 220L
    ...
    fun capabilities(requestId: String? = null): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("productAudioAuthority", "WEB_PRODUCT")
        }
    }
    ...
    private fun recognizerIntent(language: String): Intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
    ...
    private fun beginRecognizerSession(delayMs: Long) {
        ...
        if (stopRequested || recognizerSessionActive) return
        val runnable = Runnable {
            ...
            try {
                recognizer?.startListening(recognizerIntent(current.language))
        ...
        emitStatus("PENDING", if (delayMs > 0) "rearming" else "starting")
    }
    ...
    private fun finalizeProductStop() {
        ...
        finishRunnable = null
        if (accumulator.hasActiveSession) {
            accumulator.endUserFinishFallback(SystemClock.elapsedRealtime())
        }
        recognizerSessionActive = false
    ...
    override fun onReadyForSpeech(params: Bundle?) {
        emitStatus("DRAFT", "listening")
    }

    override fun onBeginningOfSpeech() {
        emitStatus("DRAFT", "speech")
    }
    ...
    override fun onEndOfSpeech() {
        emitStatus("DRAFT", "provider-endpoint")
    }

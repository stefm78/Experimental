...
const policy = json('offline-interview/native/asr-benchmark/fr-FR-v1-scoring-v2.json');
...
const benchmark = read('offline-interview/native/android-stt-poc-v1/app/src/main/java/com/stefm78/offlineinterview/nativepoc/NativeAsrBenchmarkV2Activity.kt');
...
assert.match(manifest, /android:name="\.NativeAsrBenchmarkV2Activity"/);
assert.match(manifest, /android:label="00 Native ASR Benchmark v2"/);
assert.match(gradle, /versionCode\s*=\s*16/);
assert.match(gradle, /versionName\s*=\s*"0\.6\.1-native-asr-benchmark-v2-tactical"/);
assert.match(benchmark, /ANDROID_SYSTEM_DEFAULT/);
assert.match(benchmark, /SpeechRecognizer\.createSpeechRecognizer/);
assert.match(benchmark, /offline-interview\.native-asr-benchmark-result\.v2/);
assert.match(benchmark, /completionAuthority/);
assert.match(benchmark, /USER_BUTTON/);
assert.match(benchmark, /APPEND_AND_AUTO_REARM/);
assert.match(benchmark, /EXTRA_SEGMENTED_SESSION/);
...
assert.match(workflow, /offline-interview-android-native-asr-benchmark-v2-TACTICAL-REINSTALL-ONLY/);
assert.match(workflow, /INSTALL_MODE=UNINSTALL_THEN_REINSTALL/);
assert.match(directLink, /URLSearchParams|searchParams/);

console.log('PASS beta productization contract: native-ASR v2 keeps the frozen corpus and system microphone recognizer while making passage completion user-controlled and endpointing-resilient.');

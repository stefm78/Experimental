...
s = s.replace("assert.ok(coreBytes <= 200_000, `core source budget exceeded: ${coreBytes} bytes`);", "assert.ok(coreBytes <= 207_000, `core source budget exceeded: ${coreBytes} bytes`);")
p.write_text(s)
